...
from hd_common import (BACKBONES, CLASSES, DEVICE, OUT, build, extract, gbif_splits,
                       deepweeds_all, loader, train_tf, eval_tf, metrics)
...
def run(name):
    ...
    t0 = time.time()
    (ftr, ytr), (fva, yva), (fte, yte) = gbif_splits()
    fdw, ydw = deepweeds_all()
    accum = ACCUM.get(name, 1)
    tr = loader(ftr, ytr, train_tf, True, BATCH // accum)
    va = loader(fva, yva, eval_tf, False, 64)
    print(f"[{name}] GBIF train {len(ftr)} / val {len(fva)} / test {len(fte)} | DeepWeeds {len(fdw)}", flush=True)
    ...
    X, L = {}, {}
    for split, files in [("tr", ftr), ("va", fva), ("te", fte), ("dw", fdw)]:
    ...
    np.savez(feat_path, **{f"X{s}": X[s].astype(np.float16) for s in X},
             ytr=ytr, yva=yva, yte=yte, ydw=ydw)

    res = dict(backbone=name, best_epoch=best_ep, best_val_loss=round(best, 4),
               feat_dim=int(X["tr"].shape[1]),
               gbif_test=metrics(yte, L["te"].argmax(1)),
               deepweeds=metrics(ydw, L["dw"].argmax(1)),
               seconds=round(time.time() - t0, 1), classes=CLASSES)
    res["gap_acc"] = round(res["gbif_test"]["acc"] - res["deepweeds"]["acc"], 4)
    json.dump(res, open(e2e_path, "w"), indent=2)
    print(f"[{name}] E2E  GBIF acc={res['gbif_test']['acc']:.4f}  DeepWeeds acc={res['deepweeds']['acc']:.4f}"
          f"  gap={res['gap_acc']:+.4f}  ({res['seconds']:.0f}s)", flush=True)
